...
const CONTRACT = new URL('../docs/operations/v2-r1-step5-privileged-bootstrap-design.md', import.meta.url);

const EXPECTED_CONTRACT = `# V2 r1 privileged-bootstrap review contract
...
test('Step 5 privileged-bootstrap design is the exact reviewed canonical document', async () => {
  const source = await readFile(CONTRACT, 'utf8');

  assert.equal(source, EXPECTED_CONTRACT);
});

test('canonical comparison rejects a substituted revision or implied host authority', () => {
  const bypass = EXPECTED_CONTRACT
    .replace('53530125ea25e286dd26c08b215f2b8097386af2', '53530125ea25e286dd26c08b215f2b8097386af3')
    .replace('Step 5 authorizes no host command', 'Step 5 authorizes a host command')
    .concat('sudo bootstrap\n');

  assert.notEqual(bypass, EXPECTED_CONTRACT);
});

// This test reads repository text only. It does not inspect host paths, runtime, credentials, endpoints, or external systems.
