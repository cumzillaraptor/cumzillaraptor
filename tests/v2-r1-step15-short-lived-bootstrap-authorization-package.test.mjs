...
const CONTRACT = new URL('../docs/operations/v2-r1-step15-short-lived-bootstrap-authorization-package.md', import.meta.url);

const EXPECTED_CONTRACT = `# V2 r1 Step 15 short-lived host-bootstrap authorization-package contract
...
test('Step 15 short-lived authorization-package contract is the exact reviewed canonical document', async () => {
  validateContract(await readFile(CONTRACT, 'utf8'));
});

test('canonical comparison rejects scope, expiry, binding, evidence, and authority weakening', async () => {
  const source = await readFile(CONTRACT, 'utf8');
  const mutations = [
    source.replaceAll('401369f3ec11b2ec164a049f9dc4dfd7154d3354', '0'.repeat(40)),
    source.replace('exactly one isolated, no-send r1 candidate-bootstrap action', 'multiple bootstrap actions'),
    source.replaceAll('exactly 60 minutes', 'up to 24 hours'),
    source.replace("The package `expires-at` field must equal that record's `expires-at`, and that record's `expires-at` must equal its `issued-at + 60 minutes`; otherwise creation or review must fail closed.", 'The package may choose its expiry independently.'),
    source.replace('The package cannot extend, renew, repair, or replace that interval.', 'The package may renew its interval.'),
    source.replace('each by its own lowercase SHA-256 value', 'with one shared digest'),
    source.replace('it is reported evidence only, not independently verified host proof.', 'it is host proof.'),
    source.replace('A generated or reviewed Step 15 package is not authorization to execute a bootstrap action.', 'A generated package authorizes bootstrap execution.'),
    source.replace('it cannot be refreshed or reused.', 'it may be reused.'),
  ];
  for (const mutation of mutations) assert.throws(() => validateContract(mutation));
});

// This test reads repository text only. It creates no package, digest, nonce, signature, host state, credential, endpoint, network request, or external-system action.
