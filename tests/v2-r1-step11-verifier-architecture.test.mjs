...
const CONTRACT = new URL('../docs/operations/v2-r1-step11-verifier-architecture.md', import.meta.url);

const EXPECTED_CONTRACT = `# V2 r1 Step 11 future package-verifier architecture contract
...
test('Step 11 verifier architecture is the exact reviewed canonical document', async () => {
  assert.equal(await readFile(CONTRACT, 'utf8'), EXPECTED_CONTRACT);
});

test('canonical comparison rejects Step 10 substitution and implied bootstrap authority', async () => {
  const source = await readFile(CONTRACT, 'utf8');
  const bypass = source
    .replaceAll('6aa687adf63800040bcf1e3ccfcb2c2d1799db82', '0'.repeat(40))
    .replace('success result is still not bootstrap', 'success result is bootstrap');
