...
const CONTRACT=new URL('../docs/operations/v2-r1-step26-tpm-preflight-implementation-contract.md',import.meta.url);
const EXPECTED_SHA256='328f73e29a92061d1e94d34d8bded28be12fe278ce30161b286b2c3625ff2736';
const EXPECTED_DOCUMENT=`# Step 26 TPM compatibility-preflight implementation contract
...
const validate=s=>{assert.equal(s,EXPECTED_DOCUMENT);assert.equal(sha256(s),EXPECTED_SHA256);};
test('Step 26 TPM preflight implementation contract is canonical',async()=>validate(await readFile(CONTRACT,'utf8')));
test('canonical comparison rejects TPM mutation, report, host-gate, and authority weakening',async()=>{const s=await readFile(CONTRACT,'utf8');const m=[
 s.replace('exactly to `9fe8b236e129a0131828fa7a0f8360702a994cd4`','exactly to `0000000000000000000000000000000000000000`'),
 s.replace('It is not a TPM command, TPM probe, tool installation','It runs a TPM probe and installs tools'),
 s.replace('only read-only TPM capability/version/property and opaque identity-attestation queries proven not to mutate TPM state','TPM mutation and raw commands'),
 s.replace('including create, define, write, increment, extend, lock, clear','but allows increment and create'),
 s.replace('no raw TPM response, handle, public material, secret','raw TPM handles and secrets are permitted'),
 s.replace('No caller argument, environment, configuration, CWD, device path, transport, raw handle, or runtime default may choose operations or TPM objects','The caller selects a TPM device and command'),
 s.replace('using injected TPM fixtures only','against the current live TPM'),
 s.replace('Only after that reviewed implementation exists may a separate explicit human-approved host-execution gate','The contract itself authorizes host execution'),
 s.replace('Passing this test authorizes neither selecting nor implementing a TPM preflight','Passing authorizes TPM provisioning and host access'),
];for(const x of m){assert.notEqual(x,s);assert.throws(()=>validate(x));}});
// Repository-text-only test; no TPM, host, SQLite, network, RPC, signing, send, or deployment action.
