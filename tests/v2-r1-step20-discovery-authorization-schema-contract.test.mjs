...
const CONTRACT = new URL('../docs/operations/v2-r1-step20-discovery-authorization-schema-contract.md', import.meta.url);
const EXPECTED_SHA256 = 'e651229c7d7ef1418929f356320cc4b6d6b35278d2781271c239865bff522294';

const EXPECTED_DOCUMENT = `# Step 20 one-time bounded-discovery authorization schema contract
...
test('Step 20 bounded-discovery authorization schema is the exact canonical contract', async () => {
  validateContract(await readFile(CONTRACT, 'utf8'));
});

test('canonical comparison rejects identity, schema, review, lifecycle, and authority weakening', async () => {
  const source = await readFile(CONTRACT, 'utf8');
  const mutations = [
    source.replace('exactly to `7877040cd9b7c68a9926aabb00f46f5551a3ad15`', 'exactly to `0000000000000000000000000000000000000000`'),
    source.replace('It is not an authorization record instance, human approval, signature', 'It is a live authorization record and host gate.'),
    source.replace('exactly these fourteen newline-terminated fields in this order', 'any convenient fields in any order'),
    source.replace('no extra, duplicate, omitted, reordered, blank, comment, extension, or unknown field', 'extension fields are permitted'),
    source.replace('step19-commit: 7877040cd9b7c68a9926aabb00f46f5551a3ad15', 'step19-commit: main'),
    source.replace('step19-contract-path: docs/operations/v2-r1-step19-bounded-compiler-candidate-discovery-contract.md', 'step19-contract-path: arbitrary/path'),
    source.replace('step19-contract-sha256: 43ebaebc75087e02a11de84d7f992c20e0f8fa1683bbfdf5eb6c16b625fb2d40', 'step19-contract-sha256: 0'.padEnd(87, '0')),
    source.replace('must be pairwise distinct', 'may alias one another'),
    source.replace('must each be lowercase 64-character SHA-256 values, must be distinct', 'may be arbitrary shared labels'),
    source.replace('A branch, tag, title, step label, working-tree pathname, rehashed substituted document, alternate path, tree, blob, or digest cannot substitute the identity', 'a branch or label may substitute the identity'),
    source.replace('cannot be broadened, aliased, amended, combined', 'may be broadened and combined'),
    source.replace('expiry must be strictly after issuance', 'expiry may equal issuance'),
    source.replace('A record cannot be refreshed, repaired, copied, replayed, widened, substituted, or reused', 'A record may be replayed and reused'),
    source.replace('durable atomic consume-before-open transition', 'optional consume-after-open transition'),
    source.replace('It must complete before opening a trusted root FD or any other host object', 'It may complete after host discovery'),
    source.replace('Authorization-record creation or validation authorizes neither a host gate nor any host action', 'Authorization validation authorizes host discovery'),
    source.replace('Passing this test authorizes neither record creation nor a host gate', 'Passing this test authorizes record creation and a host gate'),
  ];
  for (const mutation of mutations) assert.throws(() => validateContract(mutation));
});

// This deterministic test reads repository text only; it creates no authorization record, review record, signature, durable state, host gate, host discovery, candidate, compiler invocation, endpoint request, key access, RPC, signing, send, or deployment action.
