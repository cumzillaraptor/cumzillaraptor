...
const documentPath = path.join(root, 'docs', 'operations', 'step3-v5-read-only-metadata-probe-contract.md');
const EXPECTED_DOCUMENT = `# Step 3 v5 future read-only metadata-probe contract
...
`;
const EXPECTED_SHA256 = 'ce88574f3a1167ff1de24b131abdddd43d3682dcb08ab83c200fe845f79160a5';
...
function assertCanonical(source) {
  assert.equal(source, EXPECTED_DOCUMENT, 'document bytes differ from the reviewed canonical contract');
  assert.equal(sha256(source), EXPECTED_SHA256, 'document digest differs from the reviewed canonical contract');
}

test('Step 3 v5 future read-only metadata-probe contract is byte-exact and fail-closed', async () => {
  assertCanonical(await readFile(documentPath, 'utf8'));
});

test('Step 3 v5 metadata-probe contract mutations reject scope, report, or authority weakening', async () => {
  const source = await readFile(documentPath, 'utf8');
  for (const mutation of [
    source.replace('ed7b55f1a3c858ee4ba40aa2e4cd91890458a23e', '0'.repeat(40)),
    source.replace('3fee0ea99385dbf733b86ad21aa836968383ed7a0be541c8288f85e4a73cc2cb', '0'.repeat(64)),
    source.replace('c4d775b59f81636407488ca858fb2bf318f73b34cc3a8b41cd477a435366d570', '0'.repeat(64)),
    source.replace('selects no host path', 'selects a host path'),
    source.replace('requires a separate explicit human host-gate approval', 'requires no approval'),
    source.replace('separate immutable authorization record', 'mutable caller record'),
    source.replace('non-dereferencing existence, file type, numeric uid, numeric gid, and octal mode', 'file contents and metadata'),
    source.replace('must never read file contents', 'may read file contents'),
    source.replace('remain permanently excluded', 'may be reused'),
    source.replace('Linux `openat2(RESOLVE_BENEATH | RESOLVE_NO_SYMLINKS)` with held-FD `fstat`', 'ordinary pathname open'),
    source.replace('no pathname reconstruction, no `openat` fallback, no retry, and no compatibility path', 'fallback is permitted'),
    source.replace('are the only future resolution inputs', 'are optional resolution inputs'),
    source.replace('exactly one LF-terminated ASCII line and no other bytes', 'multiple output lines'),
    source.replace('A denial is exactly `DENY opaque-metadata-refusal\\n`', 'A denial may expose details'),
    source.replace('A metadata result is exactly `META regular <uid> <gid> <mode>\\n`', 'A metadata result may contain arbitrary fields'),
    source.replace('only for the one exact authorized regular leaf', 'for any leaf'),
    source.replace('produces exactly `DENY opaque-metadata-refusal\\n` without target facts', 'may expose a target'),
    source.replace('authorizes no source acquisition, stage creation, runtime refresh, `--prepare`, key access, RPC, signing, sending, or deployment', 'authorizes runtime refresh'),
    source.replace('This contract authorizes no repository publication', 'This contract authorizes repository publication'),
    `${source}\n`,
  ]) {
