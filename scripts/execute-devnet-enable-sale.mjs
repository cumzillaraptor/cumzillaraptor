#!/usr/bin/env node
// Devnet: set_claims_sale_state Setup -> Live. Authority-signed; verifies on-chain state after.
...
if (!authorityPath) { console.error('usage: node execute-devnet-enable-sale.mjs <authority-keypair.json>'); process.exit(1); }
const authority = Keypair.fromSecretKey(Uint8Array.from(JSON.parse(readFileSync(authorityPath, 'utf8'))));
if (!authority.publicKey.equals(EXPECTED_AUTHORITY)) { console.error('authority mismatch'); process.exit(1); }

const disc = (name) => createHash('sha256').update(`global:${name}`).digest().subarray(0, 8);
// Borsh enum: Setup=0, Paused=1, Live=2
const LIVE = Buffer.from([2]);
const [config] = PublicKey.findProgramAddressSync([Buffer.from('config')], PROGRAM_ID);

const connection = new Connection('https://api.devnet.solana.com', 'confirmed');
const before = await connection.getAccountInfo(config);
if (!before || before.data[264] !== 0) { console.error(`unexpected pre-state saleState=${before ? before.data[264] : 'missing'} (need 0/Setup)`); process.exit(1); }
