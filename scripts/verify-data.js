...
console.log('\n2. MINT LIST');
const mintCsv = fs.readFileSync('/home/raspberrypi/nft-collection/cumzillaraptors_solana/mint_list.csv', 'utf-8').trim().split('\n');
check(mintCsv.length === 248, `Has 247 entries + header (found ${mintCsv.length} lines)`);
...
check(uniqueMint.size === 247, `All 247 mint NFTs are unique`);

// Verify no overlap with reserve
const reserveCsv = fs.readFileSync('/home/raspberrypi/nft-collection/cumzillaraptors_solana/reserve_list.csv', 'utf-8').trim().split('\n');
const reserveNfts = [];
...
console.log('\n3. RESERVE LIST');
check(reserveCsv.length === 174, `Has 173 entries + header (found ${reserveCsv.length} lines)`);
...
const metaDir = '/home/raspberrypi/workspace-cumzillaraptor/nft-data/metadata';
...
console.log('\n5. COLLECTION JSON');
const collection = JSON.parse(fs.readFileSync('/home/raspberrypi/workspace-cumzillaraptor/nft-data/collection.json'));
...
// 6. Check merkle tree
console.log('\n6. MERKLE TREE');
const claimProofs = JSON.parse(fs.readFileSync('/home/raspberrypi/workspace-cumzillaraptor/nft-data/claim-proofs.json'));
const merkleConfig = JSON.parse(fs.readFileSync('/home/raspberrypi/workspace-cumzillaraptor/nft-data/merkle-config.json'));
const claimsByAddr = JSON.parse(fs.readFileSync('/home/raspberrypi/workspace-cumzillaraptor/nft-data/claims-by-address.json'));

check(Object.keys(claimProofs).length === 173, `173 claim proofs generated (found ${Object.keys(claimProofs).length})`);
check(merkleConfig.totalClaims === 173, 'Merkle config has 173 total claims');
...
console.log('\n7. MINT POOL ORDER');
const poolOrder = JSON.parse(fs.readFileSync('/home/raspberrypi/workspace-cumzillaraptor/nft-data/mint-pool-order.json'));
check(poolOrder.order.length === 247, `Pool has 247 items`);
check(poolOrder.seed === 42069, 'Seed is 42069');

const uniquePool = new Set(poolOrder.order);
check(uniquePool.size === 247, 'All pool IDs are unique');
...
check(totalClaimedInLookup === 173, `Total claimable NFTs in lookup: ${totalClaimedInLookup} (expect 173)`);
